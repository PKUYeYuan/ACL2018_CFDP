# coding=utf-8
# validation process, to determine the hyper parameters
import random
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for discourse_index in temp_order:
                ...
                gold_arcs = set([(dev_discourse[i][2], i, dev_discourse[i][3]) for i in range(len(dev_discourse))])
            ...
            outfile.write("dev: in the iteration " + str(epoch_num) + " the dev uas right ratio is: " + str(
            float(uas_correct) / float(all_arcs)) + ' correct_arcs:' + str(uas_correct) + ' all_arcs:' + str(
            all_arcs) + '\n')
            outfile.write("dev: in the iteration " + str(epoch_num) + " the dev las right ratio is: " + str(
            float(las_correct) / float(all_arcs)) + ' correct_las:' + str(las_correct) + ' all_arcs:' + str(
            all_arcs) + '\n')
            outfile.close()
            return float(uas_correct) / float(all_arcs)

def Write_devOut(discourse, arcs, epoch_num):
    devOutfile = open("./Result/" + "dev" + str(epoch_num) + ".txt", 'a+')
    temp_arcs = sorted(arcs, key=lambda x: x[1])
    ...
    for edu_iter in range(discourse_length):
        ...
        if temp_arcs[edu_iter][1] == edu_iter:
            ...
            devOutfile.write(outputStr)
        else:
            devOutfile.write("didn't predict this edu!ERROR!\n")
    devOutfile.write('\n')
    devOutfile.close()

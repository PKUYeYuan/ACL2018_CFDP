...
from Neural_model import train_model_save, train_model_import
import math
...
class GreedyDepParser:
    ...
    def train(self, Artical, iter_num):
        ...
        while not GreedyDepParser.terminal(conf):
            ...
            Gold_actionList.append(t_gold)
            conf = self.transition(t_gold, conf)
        seed = 547
        random.seed(seed)
        ...
        for i in range(int(math.ceil(len(ConfList)/10.0))):
            ...
            for scores_iter in range(len(scores)):
                ...
                t_p = max([0, 1, 2, 3], key=lambda p: score[p])
                if t_p == t_o:
                    train_correct += 1
                train_all += 1
        return train_correct, train_all

    def transition(self, t, conf):
        conf.action_stack.append(t)
        return self.transition_funcs[t](conf)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', help='CONLL training file', default='../../../Data/train_input_fine_pos')
    parser.add_argument('--test', help='CONLL testing file', default='../../../Data/test_input_fine_pos')
    parser.add_argument('--dev', help='CONLL testing file', default='../../../Data/dev_input_fine_pos')
    ...
    def dev(Iter):
        ...
        all_arcs, correct_arcs = 0.0, 0.0

        for gold_dev_sent in dev:  # gold_dev_sent  每个篇章
            gold_arcs = set([(gold_dev_sent[i][2], i) for i in range(len(gold_dev_sent))])
        ...
        return float(correct_arcs) / float(all_arcs)
    ...
    def test(Iter):
            ...
            for sent_iter in range(sents_length):
                ...
                if temp_arcs[sent_iter][1] == sent_iter:
                    if temp_arcs[sent_iter][0] == sents_length:
                        ...
                        outputStr += str(temp_arcs[sent_iter][0] + 1) + '|_' + '\n'
        ...
        outfile.write("test: in the iteration " + str(Iter) + " the test right ratio is: " + str(
            float(correct_arcs) / float(all_arcs)) + ' correct_arcs:' + str(correct_arcs) + ' all_arcs:' + str(
            all_arcs) + '\n')
        outfile.close()
        testOutfile.close()
        return float(correct_arcs) / float(all_arcs)
